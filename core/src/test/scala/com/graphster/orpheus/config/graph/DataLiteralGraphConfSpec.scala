...
class DataLiteralGraphConfSpec extends AnyFunSuite {
  test("basic creation") {
    val expLex = "123"
    val expDt = "integer"
    val dlit = DataLiteralGraphConf(StringValueConf(expLex), StringValueConf(expDt))
    assertResult(StringValueConf(expLex))(dlit.lex)
    assertResult(StringValueConf(expDt))(dlit.datatype)
    assertResult(dlit)(DataLiteralGraphConf(expLex, expDt))
  }
  ...
  test("column") {
    ...
    assert(df.columns.contains(dlit.name))
    val row = df.first()
