...
import com.graphster.orpheus.config.ValueConf
import com.graphster.orpheus.config.ValueConf
...
class Joiner(override val uid: String) extends Transformer with RefTable with HasValues with HasOutputColswSet {
  ...
  override def setValueMetas(value: Array[ValueConf]): this.type = {
  ...
  override def transformSchema(schema: StructType): StructType = {
    ...
    val structFields = $(outputCols).map(refSchema.fieldIndex).map(refSchema.fields(_))
