...
class NodeMarkerSpec extends AnyFunSuite {
    ...
    val pipeline = new Pipeline().setStages(
      new SQLTransformer().setStatement(
        """
          ...
          |  FROM __THIS__
          |""".stripMargin) +: nodeMarkers
    )
    val df = SparkUtils.generateDataFrame(10)(sf.pow("ix", 2).as("sq"))
    val tx = pipeline.fit(df).transform(df)
    nodeMarkers.foreach {
      nm =>
        val metadataExp = nm.getNodeMeta.toMetadata
        val fieldOpt = tx.schema.find(_.name == nm.getInputCol)
        assert(fieldOpt.nonEmpty, (nm.getNodeMeta, tx.columns.toSeq))
        val field = fieldOpt.get
        assert(field.metadata.contains(NodesKey))
        val nodeMetadatas = field.metadata.getMetadataArray(NodesKey)
        assert(nodeMetadatas.contains(metadataExp))
